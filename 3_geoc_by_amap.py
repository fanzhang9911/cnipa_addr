...
@File    ：3_geoc_by_amap.py
@IDE     ：PyCharm 
@Author  ：Fan Zhang
@Note    : Geocoding using Amap Geocoding API (https://lbs.amap.com/api/webservice/guide/api/georegeo)
'''
...
ADDRESS_CSV = Path("path/to/address_mainland_cn.csv")
AMAP_GEOCODED_CSV = Path("path/to/address_amap_geocoded.csv")
...
AMAP_API_KEY = os.getenv("AMAP_API_KEY")  # input Amap API key here
...
def clean_address(value) -> str:
    """Clean address text before geocoding."""
    if value is None:
        return ""
...
def geocode_amap(address: str) -> Dict[str, Optional[str]]:
    ...
    try:
        ...
        return {
            ...
            "longitude": longitude,
            "latitude": latitude,
            "match_level": get_xml_text(root, "level"),
...
def load_address_data() -> pd.DataFrame:
    """Load address records for Amap geocoding."""
    df = pd.read_csv(
        ADDRESS_CSV,
        encoding=ENCODING,
        dtype=str,
    ...
    required_columns = [ID_COLUMN, ADDRESS_COLUMN]

    for col in required_columns:
        if col not in df.columns:
            raise ValueError(f"Required column not found: {col}")

    df = df[[ID_COLUMN, ADDRESS_COLUMN]].copy()
    df = df[df[ID_COLUMN] != ""]
...
def main() -> None:
    """Run Amap geocoding and save the result."""
    if not AMAP_API_KEY:
        raise ValueError(
    ...
    AMAP_GEOCODED_CSV.parent.mkdir(parents=True, exist_ok=True)
    output_df.to_csv(
        AMAP_GEOCODED_CSV,
        index=False,
        encoding=ENCODING,
    ...
    print(f"Saved Amap geocoding results to {AMAP_GEOCODED_CSV}")
